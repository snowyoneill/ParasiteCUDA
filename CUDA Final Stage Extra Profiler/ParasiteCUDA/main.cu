...
// declare and bind textures to the global image arrays, and some variables used in reduction
texture<unsigned char, 1, cudaReadModeElementType> d_imageArrayTex;
texture<unsigned char, 1, cudaReadModeElementType> minTex;
texture<unsigned char, 1, cudaReadModeElementType> maxTex;
texture<unsigned char, 1, cudaReadModeElementType> r_imageArrayTex;
...
//#define TEST_INNER_LOOP
//#define INNER_LOOP_PASSES 5

//#define TEST_WRITE_TIME
//#define TEST_LOAD_TIME
//#define TEST_CONVERT_IMG_TO_ARRAY

//#define BENCHMARKING // Cant benchmark if you are doing multiple runs - timer is not an instance - it is static
//#define ELAPSED
...
int main(int argc, char* argv[])
{
	...
	// Define pointer to the CUDA device array (r_imageArray) and assign it to the global device memory - used as a temporary result array.
	...
	// Test the time taken to load an image from the hard disk
	#ifdef TEST_LOAD_TIME
	...
	else
	{
		...
		double elapsedTime = 0;
		if(initialiseTimer(&innerTimer)==0) {
			for(int i=0; i<INNER_LOOP_PASSES; i++)
			{
			startTimer(&innerTimer);
		#endif
			#ifdef TITLES
            ...
            #ifdef TEST_CONVERT_IMG_TO_ARRAY
                stopWatch imageTimer;
                int passes = 40;
                double elapsedTime=0;
                if(initialiseTimer(&imageTimer)==0) {
			...
			/* Test to see where image is data is written: ANSWER - bottom left corner to top right
			...
			*/
			...
			cudaBindTexture(NULL, d_imageArrayTex, d_imageArray, MAX_DATA_SIZE);
	...
	//----------------------------------------------------------------------------
			...
			// free the global reduction variables
			...
			cudaBindTexture(NULL, r_imageArrayTex, r_imageArray, MAX_DATA_SIZE);
			...
			*/
			...
			// test the time taken to write the processed image to the hard disk			
		...
		#ifdef TEST_INNER_LOOP
			stopTimer(&innerTimer);
			printf("%f\n", getElapsedTimeInMilli(&innerTimer));
			elapsedTime += getElapsedTimeInMilli(&innerTimer);
			}
		}
		printf("\nAverage Inner Time: %f(ms)\n\n", (elapsedTime/INNER_LOOP_PASSES));
	...
	// unbind the global image textures
	cudaUnbindTexture(d_imageArrayTex);
	cudaUnbindTexture(r_imageArrayTex);
